import { IInitialState, IItem, IShipingData, IUser } from './reducer.types';

export const getBasketTotal = (basket: IItem[]) => {
  return basket?.reduce((amount, item: IItem) => item.price * item.quantity + amount, 0);
};
...
const reducer = (state: AppState, action: Action) => {
  switch (action.type) {
    ...
    case 'INCREASE_QUANTITY_ITEM': {
      const basketToInc = [...state.basket];
    ...
    case 'DECREASE_QUANTITY_ITEM': {
      const basketToDec = [...state.basket];
